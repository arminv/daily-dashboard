...
fn parse_daily_forecast_extracts_weekdays_and_temps() {
    ...
    assert_eq!(forecast.weekdays.len(), 3);
    assert_eq!(forecast.highs.len(), 3);
    assert_eq!(forecast.lows.len(), 3);
    // Weekday names render as 3-letter abbreviations; don't pin the exact day
    // so the test stays date-stable.
    assert!(forecast.weekdays.iter().all(|w| w.len() == 3));
    assert!((forecast.highs[0] - 30.0).abs() < 1e-6);
    assert!((forecast.highs[1] - 28.5).abs() < 1e-6);
    assert!((forecast.lows[2] - 13.0).abs() < 1e-6);
}
...
fn parse_daily_forecast_missing_daily_returns_empty() {
    ...
    let forecast = parse_daily_forecast(&json);
    assert!(forecast.weekdays.is_empty());
    assert!(forecast.highs.is_empty());
    assert!(forecast.lows.is_empty());
}
...
fn parse_daily_forecast_bad_date_becomes_placeholder() {
    ...
    let forecast = parse_daily_forecast(&json);
    assert_eq!(forecast.weekdays, vec!["???".to_string()]);
    assert!((forecast.highs[0] - 1.0).abs() < 1e-6);
    assert!((forecast.lows[0] - 0.0).abs() < 1e-6);
}
...
fn parse_daily_forecast_tolerates_missing_temp_arrays() {
    ...
    let forecast = parse_daily_forecast(&json);
    assert_eq!(forecast.weekdays.len(), 1);
    assert!(forecast.highs.is_empty());
    assert!(forecast.lows.is_empty());
}
