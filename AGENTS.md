...
- **Pure parse functions** are extracted out of the fetch paths and unit-tested:
  ...
  - `wikipedia::parse_search_results`, `wikipedia::strip_search_html`, `wikipedia::apply_extracts_from_query`, `wikipedia::is_search_activation_key`, and `wikipedia::enter_action` — MediaWiki search parsing, HTML snippet stripping, batched extracts/descriptions, `/` activation, and Enter search-vs-open.
